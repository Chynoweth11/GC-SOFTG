 ...
 * Every function returns an SVG string. Charts are theme-fixed (dark) and use
 * the LCDOS ramp, so a score is the same colour everywhere in the product.
 * ==========================================================================*/
var CH = (function (U) {
  'use strict';

  var INK = '#dde6f2', INK2 = '#9fb0c6', INK3 = '#6c7d94', INK4 = '#4a5769',
      LINE = '#1c2634', LINE2 = '#263243', TEAL = '#3fd9ad', PANEL = '#0d1219';
  var SERIES = ['#3fd9ad', '#5aa9f5', '#f2b544', '#a884f3', '#ff8551', '#e879b8', '#9ad14e', '#f2545b', '#4fc3e8', '#8fa3bd'];
  ...
  function scatter(points, opts) {
    ...
    points.forEach(function (p) {
      ...
      if (p.sel) out.push('<circle cx="' + r2(cx) + '" cy="' + r2(cy) + '" r="' + (rr + 5) + '" fill="none" stroke="#fff" stroke-width="1"/>');
      out.push('<circle cx="' + r2(cx) + '" cy="' + r2(cy) + '" r="' + (rr + 4) + '" fill="' + p.color + '" opacity=".14"/>');
      out.push('<circle cx="' + r2(cx) + '" cy="' + r2(cy) + '" r="' + rr + '" fill="' + p.color + '" stroke="rgba(0,0,0,.5)" stroke-width=".5"><title>' + esc(p.label) + '</title></circle>');
      if (p.showLabel) {
        out.push('<text x="' + r2(cx) + '" y="' + r2(cy - rr - 6) + '" text-anchor="middle" font-size="9" font-family="var(--mono)" fill="' + INK2 +
          '" paint-order="stroke" stroke="rgba(6,8,12,.9)" stroke-width="3" stroke-linejoin="round">' + esc(p.label) + '</text>');
      }
      out.push('</g>');
    });
  ...
  function lines(series, opts) {
    ...
    if (opts.markX != null) {
      out.push('<line x1="' + r2(sx(opts.markX)) + '" y1="' + m.t + '" x2="' + r2(sx(opts.markX)) + '" y2="' + (m.t + ih) +
        '" stroke="' + TEAL + '" stroke-width="1" stroke-dasharray="3 3" opacity=".65"/>');
      out.push('<text x="' + r2(sx(opts.markX) + 4) + '" y="' + (m.t + 10) + '" fill="' + TEAL + '" font-size="8.5" font-family="var(--mono)" opacity=".85">' + esc(opts.markLabel || 'NOW') + '</text>');
    }
  ...
  function spark(values, opts) {
    opts = opts || {};
    var w = opts.width || 74, h = opts.height || 18, col = opts.color || TEAL;
  ...
  function heat(cols, rows, opts) {
    ...
    rows.forEach(function (r, ri) {
      ...
      r.cells.forEach(function (cell, ci) {
        var x = lw + ci * cw, col = cell.v == null ? '#1a2330' : U.scoreColor(cell.v);
  ...
  function waterfall(items, opts) {
    ...
    out.push('<text x="' + w + '" y="' + (yT + 15) + '" text-anchor="end" fill="' + TEAL + '" font-size="12.5" font-family="var(--mono)" font-weight="700">' + U.n1(total) + '</text>');
    out.push('</svg>');
    return out.join('');
  }
  ...
  return { radar: radar, hbars: hbars, scatter: scatter, lines: lines, spark: spark, gauge: gauge, heat: heat, waterfall: waterfall, vbars: vbars, SERIES: SERIES };
